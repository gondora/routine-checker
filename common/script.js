...
let GAS_URL = localStorage.getItem('LAB_GAS_ENDPOINT'); // ブラウザの記憶から読み込む

if (!GAS_URL) {
  const input = prompt('【ラボ重要機密】GASのウェブアプリURLを入力しなさい：');
  if (input && input.startsWith('https://script.google.com')) {
    localStorage.setItem('LAB_GAS_ENDPOINT', input);
    GAS_URL = input;
    alert('URLを観測窓（ブラウザ）に記憶させたわ。次からは入力不要よ。');
  } else {
    alert('URLが正しくないわ。これじゃあ観測データを送信できないじゃない！');
  }
}
...
function getTargetData() {
  ...
  // ① 一番上に表示したい特殊任務 (extraTop) を先頭に入れる
  if (data.extraTop && data.extraTop[day]) {
    finalQuestions = finalQuestions.concat(data.extraTop[day]);
  }

  // ② いつもの共通任務 (common) をその後ろに合体させる
  if (data.common) {
    finalQuestions = finalQuestions.concat(data.common);
  }

  // ③ 最後に、今まで通りの後ろに表示したい特殊任務 (extra) を合体させる
  if (data.extra && data.extra[day]) {
    finalQuestions = finalQuestions.concat(data.extra[day]);
  }
...
function resetLocalData() {
  if (confirm('リセットしますか？')) {
    const transaction = db.transaction(['photos'], 'readwrite');
    transaction.objectStore('photos').clear().onsuccess = () =>
      location.reload();
  }
}
